    ...
    <div class="app-container">
        <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="90px">
            ...
            <el-form-item label="判定结果" prop="result">
                <el-select v-model="queryParams.result" placeholder="请选择" style="width: 100px">
                    <el-option
        ...
        <el-table id="mytable" ref="mytable"  @row-click="clickRow" v-loading="loading" :data="picList"
            ...
            <el-table-column label="WB机台号" width="100" align="center" prop="mechineName"/>
            <el-table-column label="弹坑图片" width="350" class-name="my-cell" align="left" prop="imgPaths">
                <template #default="scope">
                    <div v-for=" (item,index) in scope.row.imgPaths" style="border: 1px solid darkgrey">
                        <el-image
            ...
            <el-table-column label="试验数量" width="80" align="center" prop="testNumber"/>
            ...
            <el-table-column label="芯片代码" width="100" align="center" prop="xpdm" />
            <el-table-column label="芯片批号" width="100" align="center" prop="xpph" />
        ...
        <!-- 添加或修改弹坑图片对话框 -->
        <el-dialog :title="title" v-model="open" @open="dialogOpen" :close-on-click-modal="false" :close-on-press-escape="false" width="900px"
                   style="min-height:350px;max-height: 80vh;overflow: scroll">
            <el-form ref="picRef" :model="form" :rules="rules" label-width="100px">
                <el-form-item>
                    <el-form-item label="流程单号" prop="flowno">
                        <el-input ref="dialog_flowno_input" v-model="form.flowno" @keyup="getFlowInfo" placeholder="请扫码/输入流程单号"/>
                    </el-form-item>
                    <el-form-item label="机台号" prop="mechineName">
                        <el-autocomplete
                                class="inline-input"
                ...
                <el-form-item>
                    <el-form-item label="试验数量" prop="testNumber">
                        <el-input type="number" v-model="form.testNumber" placeholder="手动输入/自动赋值"  />
                    </el-form-item>
                    <el-form-item label="试验时机" prop="testCharge">
                ...
                <el-form-item>
                    <el-form-item label="芯片代码" prop="xpdm">
                        <el-input v-model="form.xpdm" placeholder="自动赋值" :disabled="true" readonly/>
                    </el-form-item>
                    <el-form-item label="芯片批号" prop="xpph">
                        <el-input v-model="form.xpph" placeholder="自动赋值" :disabled="true" readonly/>
                    </el-form-item>
                </el-form-item>

                <el-form-item label="图片" prop="imgPaths">
                    <el-input style="display:none" v-model="form.imgPaths" placeholder="请输入图片" disable/>
                    <el-upload
                            ref="upload"
                            :auto-upload="false"
                            multiple
                        ...
                        <i class="el-icon-plus"><img height="100%" width="100%"
                                                     src="../../../assets/icons/svg/add1.svg"/></i>
                    </el-upload>
                </el-form-item>
                <el-form-item>
                    <div class="dialog-footer">
                        <div v-show="isUploading" style="    font-size: 20px;font-weight: bold;color: yellowgreen;">正在上传图片，请稍后。。。</div>
                        <div v-show="!isUploading">
                            <el-button type="primary" @click="uploadImgToServer" >保 存</el-button>
...
<script setup name="Pic">
    import {delPicFile,uploadImages,getHandlerList,getMechineList, getFlowBillInfo, listPic, getPic, delPic, addPic, updatePic} from "@/api/tk_custom/pic";
    ...
    const ids = ref([]);
    const testResult = ref([{label:"OK",value:"OK"},{label:"NG",value:"NG"}]);
    ...
    const proxyPath=ref(["/prod-api"]);
    // const proxyPath=ref(["/dev-api"])
    const data = reactive({
        form: {},
        queryParams: {
            pageNum: 1,
...
function getHandlerList_pic() {
    ...
    function uploadChange(file,fileList) {
        for(var i=0;i<fileList.length;i++){
            if(uploadArr.value.indexOf(fileList[i].name)==-1&&fileList[i].status!="success")
...
function uploadImg(content) {
    ...
    function handleRemove(file, fileList) {
        const str = file.url;
        form.value.imgPaths.splice(form.value.imgPaths.indexOf(str.replace("dkPic","dkPic_zip")), 1);
    ...
    function getList() {
        ...
        if (null != daterangeCreatedate && '' != daterangeCreatedate&&daterangeCreatedate.value.length>0) {
        ...
        if (null != daterangeModifydate && '' != daterangeModifydate&&daterangeModifydate.value.length>0) {
        ...
        listPic(queryParams.value).then(response => {
            ...
            for (var i = 0; i < picList.value.length; i++) {
                ...
                if (str)
                    picList.value[i].imgPaths = str.split(",");
                else
                    picList.value[i].imgPaths =[];
                if (str1)
                    picList.value[i].imgSrcPaths = str1.split(",");
                else
                    picList.value[i].imgSrcPaths =[];
            }
        ...
        getPic(_id).then(response => {
            ...
            if (str) {
                ...
                for (var i = 0; i < form.value.imgPaths.length; i++) {
                    uploadFileList.value.push({name: "", url: form.value.imgPaths[i].replace("/dkPic_zip", "/dkPic")})
                }
            }else {
                form.value.imgPaths=[];
            }
...
function uploadImgToServer() {
    ...
    function handleDelete(row) {
        ...
        proxy.$modal.confirm('是否确认删除弹坑图片编号为"' + _ids + '"的数据项？').then(function () {
            return delPic(_ids);
        }).then(() => {
            getList();
    ...
    function handleExport() {
        proxy.$modal.confirm("是否按搜索条件导出，最大导出量为200条？", "警告", {
            confirmButtonText: "确认",
            cancelButtonText: "取消",
            showclose: true,//是否显示右上角关闭按钮
            type: "warning"//提示类型
        }).then(function () {
            proxy.download('tk_custom/pic/export', {
                ...queryParams.value
...
function handleSelectExport() {
    ...
    proxy.$modal.confirm("是否按选中导出？", "警告", {
        confirmButtonText: "确认",
        cancelButtonText: "取消",
        showclose: true,//是否显示右上角关闭按钮
        type: "warning"//提示类型
    }).then(function () {
        proxy.download('tk_custom/pic/exportSelect', {remark:str}, `pic_${new Date().getTime()}.xlsx`)
    }).catch(function (e) {
    });
